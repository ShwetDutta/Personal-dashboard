...
export default function GoalsPage() {
  ...
  const [isAddingMilestone, setIsAddingMilestone] = useState<{
    [key: string]: boolean;
  }>({});
  ...
  const handleAddMilestone = async (goalId: string) => {
    ...
    setMilestoneErrors((prev) => ({ ...prev, [goalId]: "" }));
    setIsAddingMilestone((prev) => ({ ...prev, [goalId]: true }));
    ...
    try {
      ...
      setMilestoneErrors((prev) => ({
        ...prev,
        [goalId]: err?.message || "Failed to add milestone. Please try again.",
      }));
    } finally {
      setIsAddingMilestone((prev) => ({ ...prev, [goalId]: false }));
    }
  };
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-12 px-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px] mt-2 ml-5">
            Your objectives • {goals.length} active
          </p>
        </div>
        <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          {goals.map((goal) => {
            ...
            return (
              <GlassCard
                key={goal.id}
                className="p-10 flex flex-col group relative rounded-[3rem] border-white/80 transition-all duration-700 hover:bg-white overflow-hidden"
              >
                        ...
                        {goal.deadline && (
                          <div className="flex items-center gap-1.5">
                            ...
                            >
                              {daysLeft !== null
                                ? daysLeft < 0
                                  ? `Late by ${Math.abs(daysLeft)} days`
                                  : `${daysLeft} days left`
                                : ""}
                            </span>
                          </div>
                ...
                <div className="flex-1 space-y-4 relative z-10">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-300">
                      ...
                      {goalMilestones.length} completed
                    </span>
                  </div>
                  ...
                  <div className="flex gap-2 mt-6 p-2 bg-slate-50/50 rounded-2xl border border-white/40">
                    <input
                      ...
                      disabled={isAddingMilestone[goal.id]}
                    />
                    <button
                      onClick={() => handleAddMilestone(goal.id)}
                      disabled={isAddingMilestone[goal.id]}
                      className="w-10 h-10 flex items-center justify-center bg-slate-900 text-white rounded-xl hover:bg-slate-800 active:scale-95 transition-all shadow-xl shadow-slate-100 disabled:opacity-50"
                  ...
                  {milestoneErrors[goal.id] && (
                    <p className="text-xs text-rose-500 font-bold mt-2 ml-2">
                      {milestoneErrors[goal.id]}
                    </p>
                  )}
                </div>
      ...
      {isFormOpen && (
        <GoalForm onClose={() => setIsFormOpen(false)} onSubmit={addGoal} />
      )}
    </div>
  );
}
